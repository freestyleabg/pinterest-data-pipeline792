...
def send_to_endpoint(method: str, data: dict, invoke_url: str):
    ...
    response = requests.request("POST", invoke_url, data=payload, headers=headers)
    ...
    while True:
        ...
        with engine.connect() as connection:
            ...
            for key, result in data.items():
                p = Process(
                    target=send_to_endpoint, args=("POST", result, endpoints[key])
                )
                p.start()
                processes.append(p)
            ...
            # # Print results
            # for key, result in data.items():
            #     print(f"key: {key}, endpoint_url: {endpoints[key]}")
